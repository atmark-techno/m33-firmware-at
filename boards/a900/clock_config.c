...
void BOARD_BootClockRUN(void)
{
     ...
     * FRO192CLK, div is 1, so 192 MHz / (1 + 1) = 96 MHz) */
                                           ...
                                           int *slow_clk_divider, drive_mode_e drive_mode)
{
    ...
    const rtd_sys_clk_src_and_clk_name_t clk_src_and_clk_name[] = {
        { kCGC_RtdSysClkSrcFro, kCLOCK_FroClk },           { kCGC_RtdSysClkSrcPll0Pfd0, kCLOCK_Pll0Pfd0Clk },
        { kCGC_RtdSysClkSrcPll1Pfd0, kCLOCK_Pll1Pfd0Clk }, { kCGC_RtdSysClkSrcSysOsc, kCLOCK_SysOscClk },
        { kCGC_RtdSysClkSrcRtcOsc, kCLOCK_RtcOscClk },     { kCGC_RtdSysClkSrcLvds, kCLOCK_LvdsClk },
...
drive_mode_e BOARD_SwitchToFROClk(drive_mode_e drive_mode)
{
    ...
    if (((CGC_RTD->CM33CLK & CGC_CM33CLK_SCS_MASK) >> CGC_CM33CLK_SCS_SHIFT) != kCGC_RtdSysClkSrcFro)
    {
        ...
        new_drive_mode =
            BOARD_CalculateCoreClkDivider((cgc_rtd_sys_clk_src_t)g_sysClkConfigFroSource.src, &core_clk_divider,
                                          &bus_clk_divider, &slow_clk_divider, drive_mode);
...
int32_t BOARD_UpdateM33CoreFreq(cgc_rtd_sys_clk_config_t *config)
{
    ...
    for (i = 0; i < ARRAY_SIZE(clk_src); i++)
    {
        if (clk_src[i] == kCGC_RtdSysClkSrcLvds)
        {
            ...
            if (i == DRIVE_MODE_OD)
            {
                /* Max freq of Pll1Pfd0 is 432 MHz, the frequency is 423.134608 MHz = (18 * 540.672) / 23(In
                 * BOARD_InitPlls()] */
                clk_src[i] =
                    kCGC_RtdSysClkSrcPll1Pfd0; /* Use the clock source to provide higher frequency for cortex-m33 */
            }
        }
    }
     ...
     * 38.4 MHz  < m33 core frequency <= 160 MHz: 1.05 V w/AFBB
    ...
    if (status == 0)
    {
        if (m33_dest_core_frequency < SystemCoreClock)
        {
            ...
            if (SystemCoreClock > CM33_CORE_MAX_FREQ_ND && SystemCoreClock <= CM33_CORE_MAX_FREQ_OD)
            {
                ...
                if (m33_dest_core_frequency > CM33_CORE_MAX_FREQ_ND && m33_dest_core_frequency <= CM33_CORE_MAX_FREQ_OD)
                {
                         ...
                         m33_dest_core_frequency <= CM33_CORE_MAX_FREQ_ND)
                {
                    ...
                    CLOCK_SetCm33SysClkConfig(config);
                    if (voltage != PMIC_BUCK2_VOLTAGE_1_05V)
                    {
                        /* update bias configuration */
                        status = UPOWER_ChngRTDDomBias(DRIVE_MODE_ND);
                        assert(status == 0);
                        status = UPOWER_ChngPmicVoltage(PMIC_BUCK2, PMIC_BUCK2_VOLTAGE_1_05V);
                        if (status == 0)
                        {
                            status = UPOWER_GetPmicVoltage(PMIC_BUCK2, &voltage);
                            if (status == 0 && voltage == PMIC_BUCK2_VOLTAGE_1_05V)
                            {
                            }
                            else
                            {
                ...
                else if (m33_dest_core_frequency > 0 && m33_dest_core_frequency <= CM33_CORE_MAX_FREQ_UD)
                {
                    config->src = clk_src[DRIVE_MODE_UD];
                    CLOCK_SetCm33SysClkConfig(config);
                    if (voltage != PMIC_BUCK2_VOLTAGE_0_95V)
                    {
                        /* update bias configuration */
                        status = UPOWER_ChngRTDDomBias(DRIVE_MODE_UD);
                        assert(status == 0);
                        status = UPOWER_ChngPmicVoltage(PMIC_BUCK2, PMIC_BUCK2_VOLTAGE_0_95V);
                        if (status == 0)
                        {
                            status = UPOWER_GetPmicVoltage(PMIC_BUCK2, &voltage);
                            if (status == 0 && voltage == PMIC_BUCK2_VOLTAGE_0_95V)
                            {
                            }
                            else
                            {
            ...
            {
                /* 38.4 MHz < next core clk for cortex-m33 <= 160 MHz */
                if (m33_dest_core_frequency > CM33_CORE_MAX_FREQ_UD && m33_dest_core_frequency <= CM33_CORE_MAX_FREQ_ND)
                {
                    config->src = clk_src[DRIVE_MODE_ND];
                    CLOCK_SetCm33SysClkConfig(config);
                }
        ...
        {
            /* Increase voltage from 1.05 V to 1.1 V before increasing cortex-m33's clock frequency */
...
void BOARD_SwitchDriveMode(void)
{
     ...
     *       |      |
     *       ---<----
     * UD: 0.90 V (only FRO is available for RTD in Under Drive Mode)
     * ND: 1.05 V
     * OD: 1.10 V
     */
    // clang-format on
    int voltage[] = { PMIC_BUCK2_VOLTAGE_0_95V, PMIC_BUCK2_VOLTAGE_1_05V, PMIC_BUCK2_VOLTAGE_1_1V };
