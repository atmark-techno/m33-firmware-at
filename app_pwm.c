...
static void pwm_init_device(hal_pwm_handle_t *halPwmHandle, uint8_t chipId)
{
    switch (chipId)
    {
        ...
        case 2:
            ...
            HAL_PwmInit(halPwmHandle[2], 2U, CLOCK_GetIpFreq(kCLOCK_Tpm2));
            break;
        case 3:
            ...
            HAL_PwmInit(halPwmHandle[3], 3U, CLOCK_GetIpFreq(kCLOCK_Tpm3));
            break;
    }
}
...
{
    srtm_pwm_adapter_t pwmAdapter = SRTM_PwmAdapter_Create(pwmHandles, ARRAY_SIZE(pwmHandles), pwm_init_device);
    assert(pwmAdapter);
