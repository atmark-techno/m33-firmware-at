...
#define SRTM_AUDIO_CMD_INIT (0x18U)
...
static void SRTM_AudioService_HandlePeriodDone(srtm_dispatcher_t dispatcher, srtm_audio_dir_t dir,
                                               srtm_audio_service_t handle, uint32_t index)
{
...
static srtm_status_t SRTM_AudioService_PeriodDone(srtm_service_t service, srtm_audio_dir_t dir, uint8_t index,
                                                  uint32_t periodIdx)
{
...
static srtm_status_t SRTM_AudioService_BindChannel(srtm_service_t service, srtm_sai_adapter_t sai,
                                                   srtm_channel_t channel);
static srtm_status_t SRTM_AudioService_AddAudioInterface(srtm_audio_service_t handle, srtm_sai_adapter_t sai);
static srtm_status_t SRTM_AudioService_Init(srtm_service_t service, uint8_t busID, srtm_channel_t channel)
{
    srtm_audio_service_t handle = (srtm_audio_service_t)(void *)service;
    srtm_sai_adapter_t saiAdapter;
    srtm_status_t status;

    saiAdapter = handle->init(busID, channel);
    if (!saiAdapter)
        return SRTM_Status_Error;

    status = SRTM_AudioService_AddAudioInterface(handle, saiAdapter);
    if (status != SRTM_Status_Success)
        return status;

    return SRTM_AudioService_BindChannel(service, saiAdapter, channel);
}
...
static srtm_status_t SRTM_AudioService_Request(srtm_service_t service, srtm_request_t request)
{
    ...
    else
    {
        ...
        if (iface == NULL)
        {
            if (command == SRTM_AUDIO_CMD_INIT)
            {
                status = SRTM_AudioService_Init(service, audioRespBuf[0], channel);
                if (status == SRTM_Status_Success)
...
static srtm_audio_iface_t SRTM_AudioService_CreateIface(srtm_audio_service_t handle, uint8_t index,
                                                        srtm_sai_adapter_t sai, srtm_codec_adapter_t codec)
{
...
srtm_service_t SRTM_AudioService_Create(srtm_audio_init_t init)
{
