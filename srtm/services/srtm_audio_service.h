...
enum srtm_audio_retcodes
{
    SRTM_AUDIO_RETCODE_SUCCESS,
    SRTM_AUDIO_RETCODE_FAIL,
    SRTM_AUDIO_RETCODE_EBUSY,
    SRTM_AUDIO_RETCODE_UNSUPPORTED,
    SRTM_AUDIO_RETCODE_EINVAL,
    SRTM_AUDIO_RETCODE_ENOMEM,
};
...
struct _srtm_sai_adapter
{
    ...
    srtm_status_t (*setParam)(srtm_sai_adapter_t adapter, srtm_audio_dir_t dir, uint8_t index, uint8_t format,
                              uint8_t channels, uint32_t srate);
    srtm_status_t (*setBuf)(srtm_sai_adapter_t adapter, srtm_audio_dir_t dir, uint8_t index, uint8_t *bufAddr,
                            uint32_t bufSize, uint32_t periodSize, uint32_t periodIdx);
...
typedef srtm_sai_adapter_t (*srtm_audio_init_t)(uint8_t bus_id, srtm_channel_t channel);
...
/*!
 * @brief Create audio service as index 0 audio interface.
 *
