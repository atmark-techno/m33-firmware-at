...
static srtm_status_t SRTM_WdogService_Request(srtm_service_t service, srtm_request_t request)
{
    ...
    switch (command)
    {
        case WDOG_RPMSG_ENABLE:
            ...
            retCode =
                status == SRTM_Status_Success ? WDOG_RPMSG_SUCCESS : WDOG_RPMSG_ERROR;
            break;
...
out:
    response = SRTM_Response_Create(channel, SRTM_WDOG_CATEGORY, SRTM_WDOG_VERSION,
...
srtm_status_t SRTM_WdogService_NotifyPreTimeout(srtm_service_t service)
{
    ...
    srtm_notification_t notif = SRTM_Notification_Create(
                    NULL, SRTM_WDOG_CATEGORY, SRTM_WDOG_VERSION,
                    WDOG_RPMSG_NOTIFY, sizeof(*payload));
    if (!notif) {
                                       ...
                                       srtm_wdog_service_ping_t pingCb)
{
    ...
    handle = (srtm_wdog_service_t)SRTM_Heap_Malloc(
            sizeof(struct _srtm_wdog_service));
    assert(handle);
